...
#define FAC_QUICK_SET_CFG          QUICK_SET_COMFORTABLE  
...
#elif defined(__RT1_36TN__) || defined(__RT1_36WIFI__)
...
{    
    ...
    .disTimmingTimeFlag         = FAC_TIMMING_TIME_FALG,
    ...
    .quickSet                   = FAC_QUICK_SET_CFG, 
    .tempControlMode            = FAC_TIME_CON_CFG,  
    .weekPrg                    = FAC_WEEK_PRG,
    ...
    .record.comfortableTemp     = FAC_COMFORTABLE_SET_TEMP,
    .record.saveTemp            = FAC_SAVE_SET_TEMP,
...
{  
    ...
    s_sysPara.quickSet            = FAC_QUICK_SET_CFG;    
    s_sysPara.tempControlMode =   FAC_TIME_CON_CFG;
    s_sysPara.weekPrg = FAC_WEEK_PRG;   
    ...
    s_sysPara.record.comfortableTemp     = FAC_COMFORTABLE_SET_TEMP;
    s_sysPara.record.saveTemp            = FAC_SAVE_SET_TEMP;
...
{   
    ...
    app_con_tempLegalProtect(s_sysPara.record.maxSetTemp,s_sysPara.record.minSetTemp,&s_sysPara.record.comfortableTemp);
    app_con_tempLegalProtect(s_sysPara.record.maxSetTemp,s_sysPara.record.minSetTemp,&s_sysPara.record.saveTemp);      
}
//------------------------------E N D-------------------------------------------
...
void app_control_timeOut_wifiLinkHandle(void)
{
    ...
    else if((WIFI_LINK_BUSY == s_sysPara.wifi.linkStatus) || (WIFI_LINK_FAIL == s_sysPara.wifi.linkStatus)) 
    {
        if(WIFI_CONN_CLOUD == app_pullTuyaWifiStateFormThisModule())
        {
            ...
            //{
                if(TIMMING_STATUS_RUN == s_sysPara.timming.status)
                {
                    if(app_control_timeOut_get(DELAY_TIMMING))
                    {
                        ...
                        if(s_sysPara.disTimmingTimeFlag)
                        {
                            s_sysPara.disTimmingTimeFlag = false;
                        }
                        else
                        {
                            s_sysPara.disTimmingTimeFlag = true;
                        }
                    }                   
                }
                else
