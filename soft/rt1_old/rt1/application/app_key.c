...
void app_key_powOffMode(void)
{
    ...
    if(PUSH_DOWN & mod_keyOperation(KEY_POW,NOCONTINUE,NOCONTINUE))
    {
        ...
        if(TIMMING_STATUS_RUN == ptSys->timming.status)
        {
            ...
            ptSys->disTimmingTimeFlag = false;
        }                       
    }
    ...
    {
        ptSys->quickSet = QUICK_SET_COMFORTABLE;
        ptSys->runSetTemp = ptSys->record.comfortableTemp;
...
void app_key_runMode(void)
{
    ...
    {       
        ...
        else if(PUSH_CONTINUE == currentKey)
        {
            if(app_real_time_getVaildStatus())
            {
                ...
                if(PROGRAM_OFF == ptSys->weekPrg)
                {
                    ptSys->weekPrg = PROGRAM_5;
                }
            } 
    ...
    {
        ptSys->quickSet = (quickSet_t)keyOperation(DDR_ADD,\
          DATA_LOOP,1,TEMP_CONTROL_MODE_SAVE,QUICK_SET_COMFORTABLE,(int16_t)ptSys->quickSet); 
        int16_t *ptSet = &ptSys->record.comfortableTemp;
        if(TEMP_CONTROL_MODE_SAVE == ptSys->quickSet)
        {
            ptSet = &ptSys->record.saveTemp;
        }
...
void app_key_highSet_action(bool ddr)
{
    ...
    switch(ptSys->highSet.menu)
    {
        ...
        {
            #if defined (__RT1_36TN__) || defined (__RT1_36WIFI__)       
            ptSys->record.lifTemp = keyOperation(ddr,DATA_KEEP,10,MAX_LIF_TEMP,MIN_LIF_TEMP,ptSys->record.lifTemp);
            #endif
            break;
        }
        case HIGH_SETTING_COMFORTABLE_SET_TEMP:
        {
            ptSys->record.comfortableTemp = keyOperation(ddr,DATA_KEEP,5,MAX_COMFORTABLE_SET_TEMP,MIN_COMFORTABLE_SET_TEMP,ptSys->record.comfortableTemp);
            app_con_tempLegalProtect(ptSys->record.maxSetTemp,ptSys->record.minSetTemp,&ptSys->record.comfortableTemp);
            break;
        }
        case HIGH_SETTING_SAVE_SET_TEMP:
        {
            ptSys->record.saveTemp = keyOperation(ddr,DATA_KEEP,5,MAX_SAVE_SET_TEMP,MIN_SAVE_SET_TEMP,ptSys->record.saveTemp); 
            app_con_tempLegalProtect(ptSys->record.maxSetTemp,ptSys->record.minSetTemp,&ptSys->record.saveTemp);
            break;
        }
...
void app_key_highSetMode(void)
{
    ...
    {   
        ...
        ptSys->highSet.facStatus = FAC_IDLE;
        #if defined(__RT1_13__) || defined(__RT1_23WIFI__) || defined(__RT1_23__)
