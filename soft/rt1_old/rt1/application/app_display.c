...
void app_dis_tempBot(int16_t temp)
{
    ...
    mod_lcd_char(DIS_SET_TEMP_CHAR,BN_TRUE);
}
...
void app_dis_measTemp(void)
{
    ...
    if(disMeasTempFlag)
    {
        ...
        else
        {
            ...
            mod_lcd_char(DIS_MEAS_TEMP_MARK,BN_TRUE);           
        }
...
void app_dis_setTemp(int16_t setTemp,bool disFlag)
{
    if(disFlag)
    {
        ...
        mod_lcd_char(DIS_SET_TEMP_MARK,BN_TRUE);
    }   
}
...
{   
    ...
    if(disFlag)
    {
        ...
        {
            bool disFlag = true;                
            uint8_t addr[2] = {DIS_POW_OFF_MARK,DIS_POW_ON};
...
void app_dis_leftBox(void)
{
    ...
    {        
        mod_lcd_char(DIS_OUTPUT_HEAT,BN_TRUE);
    }
    else
    {
...
void app_dis_output(void)
{
    ...
    if(ptSys->floorRelayFlag)
    {      
        mod_lcd_char(DIS_OUTPUT_MARK,BN_TRUE);
    }
    else
    {
        #if defined(__RT1_36TN__) || defined(__RT1_36WIFI__)
        if(relay_out_limitFlag())
        {
            if(ptSys->blinkEvent)
            {
                mod_lcd_char(DIS_OUTPUT_MARK,BN_TRUE);
                mod_lcd_char(DIS_ALL_SENSOR,BN_TRUE);               
            }
        }
        #endif
    }
...
////                  mod_lcd_char(DIS_OUTPUT_ENABLE,BN_TRUE);
//             }
////             else
////             {
////                  if(ptSys->blinkEvent)
...
void app_dis_rightBox(void)
{
    ...
    if(QUICK_SET_COMFORTABLE == ptSys->quickSet)
    {      
        mod_lcd_char(DIS_MOD_COMFORTABLE,BN_TRUE);       
    }  
    else
    {
        mod_lcd_char(DIS_MODE_SAVE,BN_TRUE);
    }
}
...
void app_dis_powOffMode(void)
{
    ...
    #endif
    if(!ptSysPara->disTimmingTimeFlag)
    {
        mod_lcd_char(DIS_POW_OFF_MARK,BN_TRUE);
...
void app_dis_runMode(void)
{
    ...
    ptSysPara = controler_getSysParaPt();
    app_dis_measTemp();   
    app_dis_setTemp(ptSysPara->runSetTemp,!ptSysPara->disTimmingTimeFlag);
    app_dis_timming();
    app_dis_leftBox();
    app_dis_rightBox(); 
    app_dis_output();
...
void app_dis_highSetMode(void)
{
    ...
    mod_lcd_char(DIS_HIGH_SET_MARK,BN_TRUE);
    ...
    switch(ptSys->highSet.menu)
    {
        ...
        {            
            mod_lcd_char(DIS_MAX_SET_TEMP_MARK,BN_TRUE);
            disTempFlag = true;
        ...
        {            
            mod_lcd_char(DIS_MIN_SET_TEMP_MARK,BN_TRUE);
            disTempFlag = true;
        ...
        {            
            mod_lcd_char(DIS_CONTORL_DIF,BN_TRUE);
            disTempFlag = true;
        ...
        {                
            mod_lcd_char(DIS_SENSOR_MARK,BN_TRUE);
            uint8_t addr[3] = {DIS_INSIDE_SONSOR,DIS_OUTSIDE_SONSOR,DIS_ALL_SENSOR};
            ...
            if(ptSys->blinkEvent)
            {
               mod_lcd_char((rt1_lcd_char_number_t)addr[(uint8_t)ptSys->record.sensorMode],BN_FALSE);        
            }                         
            break;
        }
        ...
        {           
            mod_lcd_char(DIS_MOD_COMFORTABLE,BN_TRUE);
            disTempFlag = true;
            ptTemp = &ptSys->record.comfortableTemp;
            break;
        }
        ...
        {           
            mod_lcd_char(DIS_MODE_SAVE,BN_TRUE);
            disTempFlag = true;
            ptTemp = &ptSys->record.saveTemp;
            break;
        }
        case HIGH_SETTING_OUTPUT_OPEN_TIME:
        case HIGH_SETTING_OUTPUT_CLOSE_TIME:
        {
            uint8_t addr[2] = {DIS_OUTPUT_ENABLE,DIS_OUTPUT_STOP}; 
            mod_lcd_char(DIS_MINUTE,BN_TRUE);
            mod_lcd_char(DIS_TIME_CONROTL,BN_TRUE);           
...
{  
    ...
    digBuf[1] = CHAR_3;
    digBuf[2] = BLACK;
    #elif defined __RT1_36TN__
    digBuf[0] = CHAR_3;
    digBuf[1] = CHAR_6;
    digBuf[2] = CHAR_T;
    #elif defined __RT1_23WIFI__
    digBuf[0] = CHAR_2;
    digBuf[1] = CHAR_3;
    digBuf[2] = CHAR_I;
...
void app_dis_showAll(void)
{
    ...
    mod_lcd_char(DIS_HIGH_SET_MARK,BN_FALSE);
    mod_lcd_char(DIS_TEMP_ADJ,BN_FALSE);
    mod_lcd_char(DIS_MAX_SET_TEMP_MARK,BN_FALSE);
    mod_lcd_char(DIS_MIN_SET_TEMP_MARK,BN_FALSE);
    mod_lcd_char(DIS_CONTORL_DIF,BN_FALSE);
    mod_lcd_char(DIS_POW_ON_STATUS,BN_FALSE);
    mod_lcd_char(DIS_POW_OFF_MARK,BN_FALSE);
    mod_lcd_char(DIS_SENSOR_MARK,BN_FALSE);
    mod_lcd_char(DIS_INSIDE_SONSOR,BN_FALSE);
    mod_lcd_char(DIS_OUTSIDE_SONSOR,BN_FALSE);
    mod_lcd_char(DIS_ALL_SENSOR,BN_FALSE);
    mod_lcd_char(DIS_MINUTE,BN_FALSE);
    mod_lcd_char(DIS_OUTPUT_ENABLE,BN_FALSE);
    mod_lcd_char(DIS_OUTPUT_STOP,BN_FALSE);
