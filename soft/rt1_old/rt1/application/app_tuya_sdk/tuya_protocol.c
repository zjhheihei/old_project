...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_WORK_DAYS, DP_TYPE_ENUM},
  {DPID_VALVE_STATE, DP_TYPE_ENUM},
  {DPID_WEEK_PROGRAM1, DP_TYPE_RAW},
  {DPID_CHILD_LOCK, DP_TYPE_BOOL},
  {DPID_SENSOR_CHOOSE, DP_TYPE_ENUM},
  {DPID_FAULT, DP_TYPE_BITMAP},
  {DPID_RESERVE, DP_TYPE_RAW},
  {DPID_REG_RAW, DP_TYPE_RAW},
  {DPID_RESERVE_CHAR, DP_TYPE_STRING},
  {DPID_FLOORTEMP, DP_TYPE_VALUE},
  {DPID_FLOORSENSERTYPE, DP_TYPE_ENUM},
  {DPID_FLOORLIMITTEMP, DP_TYPE_VALUE},
  {DPID_NOTFROZEN, DP_TYPE_BOOL},
};
...
static unsigned char dp_download_mode_handle(const unsigned char value[], unsigned short length)
{
  ...
  unsigned char mode;
  #ifndef NDEBUG
  volatile static unsigned char debug_mode;
  #endif
...
static unsigned char dp_download_week_program1_handle(const unsigned char value[], unsigned short length)
{
  ...
  */
  ...
  for(i = WEEK_PROGRAM1_REG_START;i < WEEK_PROGRAM1_REG_END; i++)
  {
      app_pushContorolDataFromCloud_download(i,dat[i - WEEK_PROGRAM1_REG_START]);
  }  
  //处理完DP数据后应有反馈
