...
//--------------------------------------------------------------------------------------------------
#define max_mendred_control_reg    37
//--------------------------------------------------------------------------------------------------
static menred_controlData_def menred_controlData[max_mendred_control_reg] =
{
    ...
    {.mcd_reg_addr = regCloudAddr_linkRelayOpenDelayTime,   .mcd_reg_details = 0, .mcd_reg_stateBits = 0},
    {.mcd_reg_addr = regCloudAddr_notFrozen_set_temp,       .mcd_reg_details = 0, .mcd_reg_stateBits = 0},
};
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
INT8U make_102_reg_frame_raw(INT8U* _out_rawbuff,INT8U current_index)
{
    ...
    for(i = 0;i < max_mendred_control_reg;i++)
    {
        switch(menred_controlData[i].mcd_reg_addr)
        {
            ...
            {
                _out_rawbuff[8] = menred_controlData[i].mcd_reg_details >> 8;
                _out_rawbuff[9] = menred_controlData[i].mcd_reg_details;
                menred_controlData[i].mcd_reg_stateBits &= (~mcdRegStateBits_update_ready);
                break;
            }
            case regCloudAddr_notFrozen_set_temp:
            {
                _out_rawbuff[10] = menred_controlData[i].mcd_reg_details >> 8;
                _out_rawbuff[11] = menred_controlData[i].mcd_reg_details;
                menred_controlData[i].mcd_reg_stateBits &= (~mcdRegStateBits_update_ready);
                break;
            }
            default:break;
    ...
    for(i = 0;i < max_mendred_control_reg;i++)
    {
        ...
        { 
            ...
            switch(menred_controlData[i].mcd_reg_addr)
            {
                ...
                {
                    #ifndef NDEBUG
                    testDat = menred_controlData[i].mcd_reg_details;                    
                    #endif
                ...
                case regCloudAddr_openWindowPreheatSwitch:
                {
                    INT8U raw_buff[12];
                    INT8U length;
                    length = make_102_reg_frame_raw(raw_buff,i);
