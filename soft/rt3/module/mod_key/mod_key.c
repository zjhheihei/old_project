...
typedef enum
{
    KEY_STATUS_INIT      = 0x00,  //初始化
    KEY_STATUS_DELAY     = 0x01,  //触摸屏延时防止上电误触
    KEY_STATUS_RUN       =0x02,   
}keyStatus_t;
static keyStatus_t            keyStatus = KEY_STATUS_INIT;
void mod_key_scanTask(INT16U milliscond)
{
    ...
    switch(keyStatus)
    {
        case KEY_STATUS_INIT:
        {
            mod_key_configure();
            keyStatus = KEY_STATUS_DELAY;
            break;
        }
        case KEY_STATUS_DELAY:
        {
            if(milliscond > 3000)
            {
                keyStatus = KEY_STATUS_RUN;
                mod_key_maskOnceOperation(KeyAloneBit0);
                mod_key_maskOnceOperation(KeyAloneBit1);
                mod_key_maskOnceOperation(KeyAloneBit2);
                mod_key_maskOnceOperation(KeyAloneBit3);
                mod_key_maskOnceOperation(KeyAloneBit4);
                mod_key_maskOnceOperation(KeyAloneBit5);
                mod_key_maskOnceOperation(KeyAloneBit6);
                mod_key_maskOnceOperation(KeyAloneBit7);
            }          
            break;
        }
        case KEY_STATUS_RUN:
