...
static INT16S InSideTemperature  = 220;
static INT16S OutSideTemperature = 220;
...
void mod_ntc3380_scanTask(void)
{
    ...
    static INT8U   lastOutsideSensorType = SENSOR_TYPE_3380_10K;
            ...
            {       
                ...
                if(ADC1_SampleTimes>16)
                {
                    ...
                    if((ADC1_16thDate>4050)||(ADC1_16thDate<30)) 
                    {
                        ...
                        {                                         
                            ntcError |= 0x01;
                            InSideTemperature = 995;
                        }
                        else if(OutsideNumber == ChannelCount)
                        {                                         
                            ntcError |= 0x02;
                            OutSideTemperature = 995;
                        }
                    }
                    else
                    {
                        ...
                        {    
                            ...
                            InSideTemperature = Mod_SearchNTCTable(ADC1_16thDate,SENSOR_TYPE_3380_10K);
                            ntcError &= (~0x01);
                        }
                        else if(OutsideNumber == ChannelCount)
                        {                              
                            OutSideTemperature = Mod_SearchNTCTable(ADC1_16thDate,lastOutsideSensorType);
                            ntcError &= (~0x02);
                            if(lastOutsideSensorType != outSideSensorType)
                            {
                                lastOutsideSensorType = outSideSensorType;
                            }
                        }
                    }
                    ...
                    if(ChannelCount > MAX_ADC_CHANNEL)
                    {
