...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_WORK_MODE           ,  DP_TYPE_ENUM  },  //2
  {DPID_WORK_STATUS         ,  DP_TYPE_ENUM  },  //3
  {DPID_TEMP_SET            ,  DP_TYPE_VALUE },  //16
  {DPID_MAX_SET_TEMP        ,  DP_TYPE_VALUE },  //19
  {DPID_TEMP_CURRENT        ,  DP_TYPE_VALUE },  //24
  {DPID_MIN_SET_TEMP        ,  DP_TYPE_VALUE },  //26
  {DPID_DIF_TEMP            ,  DP_TYPE_VALUE },  //27
  {DPID_WORK_DAYS           ,  DP_TYPE_ENUM  },  //31
  {DPID_VALVE_STATE         ,  DP_TYPE_ENUM  },  //36
  {DPID_FAC                 ,  DP_TYPE_BOOL  },  //39
  {DPID_SENSOR_SELECT       ,  DP_TYPE_ENUM  },  //43
  {DPID_FAULT               ,  DP_TYPE_BITMAP},  //45
  {DPID_SET_WIND_SPEED      ,  DP_TYPE_ENUM  },  //64
  {DPID_SUN_SET_TEMP        ,  DP_TYPE_VALUE },  //101
  {DPID_MOON_SET_TEMP       ,  DP_TYPE_VALUE },  //102
  {DPID_POF                 ,  DP_TYPE_BOOL  },  //103
  {DPID_PCO                 ,  DP_TYPE_BOOL  },  //104
  {DPID_LIF                 ,  DP_TYPE_VALUE },  //105    
  {DPID_AI_PROGRAM_WARM     ,  DP_TYPE_RAW   },  //106  
  {DPID_AI_PROGRAM_COLD     ,  DP_TYPE_RAW   },  //107  
  {DPID_AI_PROGRAM_ODM      ,  DP_TYPE_RAW   },  //108  
  {DPID_OTHER_SET           ,  DP_TYPE_RAW   },  //109 
};
...
******************************************************************************/
static unsigned char dp_download_bool_handle(const unsigned char value[], unsigned short length,unsigned short dpid,unsigned short regAddr)
{ 
    unsigned char ret = TUYA_ERROR;
    unsigned char the_bool; 
    the_bool = mcu_get_dp_download_bool(value,length);
    app_pushContorolDataFromCloud_download(regAddr,the_bool);
    ret = mcu_dp_bool_update(dpid,the_bool); 
    return ret;
}

static unsigned char dp_download_enum_handle(const unsigned char value[],unsigned short length,unsigned short dpid,unsigned short regAddr)
{
    unsigned char ret;
    unsigned char the_enum;  
    the_enum = mcu_get_dp_download_enum(value,length);
    app_pushContorolDataFromCloud_download(regAddr,the_enum);  
    ret = mcu_dp_enum_update(dpid,the_enum);
    return ret;
}

static unsigned char dp_download_value_handle(const unsigned char value[],unsigned short length,unsigned short dpid,unsigned short regAddr)
{
    unsigned char ret;
    unsigned long the_value; 
    the_value = mcu_get_dp_download_value(value,length);
    app_pushContorolDataFromCloud_download(regAddr,the_value);
    ret = mcu_dp_value_update(dpid,the_value);
    return ret;
}

static unsigned char dp_download_raw_handle(const unsigned char value[], unsigned short length,unsigned short dpid,unsigned short regAddr)
{
    unsigned char ret  = TUYA_SUCCESS;     
    unsigned char i    = 0;        
    uint8_t dat[128];
    uint8_t len;
    len = length;
    if(len < 128)
    {
        ...
        raw_dp_download_from_cloud(regAddr,len,dat);       
        ret = mcu_dp_raw_update(dpid,dat,len);       
    }
    return ret;
}


//static unsigned char dp_download_char_handle(const unsigned char value[], unsigned short length)
//{
//  //示例:当前DP类型为STRING
//  unsigned char ret;
//  /*
...
/*****************************************************************************
函数名称 : dp_download_openWindowSitch_handle
功能描述 : 针对DPID_OPEN_WINDOW_SWITCH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:TUYA_SUCCESS/失败返回:ERROR
...
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
//#define WEEK_PROGRAM1_REG_START  regCloudAddr_programWorkDaysTime_1
//#define WEEK_PROGRAM1_REG_END    0X18
static unsigned char dp_download_week_program1_handle(const unsigned char value[], unsigned short length)
{
  ...
  */
  ...
  unsigned char len = (length - 2) / 2; //length 的前2位无效
  if(len > 32)
  {
      len = 32;
  }
  for(;i < len ;i++)
  {
      ...
      dat[i] <<= 8;
      dat[i]  |= (unsigned int)value[3 + i * 2];      
  }
  for(i = 0;i < 32; i++)
  {
      app_pushContorolDataFromCloud_download(i,dat[i - REG_CLOUD_ADDR_COLD_INT0_TIME]);
  }  
  //处理完DP数据后应有反馈
...
{
    unsigned char ret = TUYA_SUCCESS; 
  unsigned int regRawdat[6];
  unsigned char i = 0; 
  unsigned char len = length / 2;
  if(len > 6)
  {
      len = 6;
  }
  for(;i < len ;i++)
  {
      regRawdat[i] = (unsigned int)value[i * 2];
      regRawdat[i] <<= 8;
      regRawdat[i] |= (unsigned int)value[i * 2 + 1];      
  }
  for(i = RAW_REG_START_ADD;i < RAW_REG_END_ADD; i++)
  {
      app_pushContorolDataFromCloud_download(i,regRawdat[i - RAW_REG_START_ADD]);
  }  
 ...
 */
  ...
  if(time[0] == 1)
  {
    //正确接收到wifi模块返回的本地时钟数据 
      ...
      now_rtcTime.netRTC_second = time[6];
...
输入参数 : dpid:DP序号
           value:dp数据缓冲区地址
           length:dp数据长度
返回参数 : 成功返回:TUYA_SUCCESS/失败返回:ERRO
使用说明 : 该函数用户不能修改
*****************************************************************************/
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  ...
  ***********************************/
  ...
  unsigned char dpidType;
  switch(dpid)
  {                 
      case DPID_SWITCH           :regAddr = REG_CLOUD_ADDR_POWER_SWITCH  ; dpidType = 0x01;break; 
      case DPID_WORK_MODE        :regAddr = REG_CLOUD_ADDR_WORK_MODE     ; dpidType = 0x02;break;
      case DPID_WORK_STATUS      :regAddr = REG_CLOUD_ADDR_WORK_STATUS   ; dpidType = 0x02;break;
      case DPID_TEMP_SET         :regAddr = REG_CLOUD_ADDR_RUN_SET_TEMP  ; dpidType = 0x03;break;
      case DPID_MAX_SET_TEMP     :regAddr = REG_CLOUD_ADDR_MAX_SET_TEMP  ; dpidType = 0x03;break;
      case DPID_TEMP_CURRENT     :break;
      case DPID_MIN_SET_TEMP     :regAddr = REG_CLOUD_ADDR_MIN_SET_TEMP  ; dpidType = 0x03;break;
      case DPID_DIF_TEMP         :regAddr = REG_CLOUD_ADDR_DIF_SET_TEMP  ; dpidType = 0x03;break;   
      case DPID_WORK_DAYS        :regAddr = REG_CLOUD_ADDR_WEEK_PRG_SET  ; dpidType = 0x02;break;
      case DPID_VALVE_STATE      :regAddr = REG_CLOUD_ADDR_VALVE_STATUS  ; dpidType = 0x02;break;
      case DPID_FAC              :regAddr = REG_CLOUD_ADDR_FAC_SET       ; dpidType = 0x01;break;
      case DPID_SENSOR_SELECT    :regAddr = REG_CLOUD_ADDR_SERSON_SECELT ; dpidType = 0x02;break;
      case DPID_FAULT            :regAddr = REG_CLOUD_ADDR_FAULT         ; dpidType = 0x05;break;     
      case DPID_SET_WIND_SPEED   :regAddr = REG_CLOUD_ADDR_SET_WIND_SPEED; dpidType = 0x02;break;  
      case DPID_SUN_SET_TEMP     :regAddr = REG_CLOUD_ADDR_SUN_SET_TEMP  ; dpidType = 0x03;break;
      case DPID_MOON_SET_TEMP    :regAddr = REG_CLOUD_ADDR_MOON_SET_TEMP ; dpidType = 0x03;break;
      case DPID_POF              :regAddr = REG_CLOUD_ADDR_POF_SET       ; dpidType = 0x01;break;
      case DPID_PCO              :regAddr = REG_CLOUD_ADDR_FCO_SET       ; dpidType = 0x01;break;
      case DPID_LIF              :regAddr = REG_CLOUD_ADDR_LIF_SET_TEMP  ; dpidType = 0x03;break;
      case DPID_AI_PROGRAM_WARM  :regAddr = REG_CLOUD_ADDR_AI_PRO_WARM   ; dpidType = 0x04;break;
      case DPID_AI_PROGRAM_COLD  :regAddr = REG_CLOUD_ADDR_AI_PRO_COLD   ; dpidType = 0x04;break;
      case DPID_AI_PROGRAM_ODM   :regAddr = REG_CLOUD_ADDR_AI_PRO_OEM    ; dpidType = 0x04;break;
      case DPID_OTHER_SET        :regAddr = REG_CLOUD_ADDR_OTHER_SET     ; dpidType = 0x04;break;
      default:break;  
  }
  if(dpidType)
  {
      switch(dpidType)
      {
          case 0x01:ret = dp_download_bool_handle(value,length,dpid,regAddr)  ;break;
          case 0x02:ret = dp_download_enum_handle(value,length,dpid,regAddr)  ;break;
          case 0x03:ret = dp_download_value_handle(value,length,dpid,regAddr) ;break;
          case 0x04:ret = dp_download_raw_handle(value,length,dpid,regAddr)   ;break;
          case 0x05:/*ret = dp_download_REG_RAW_handle(value,length)        ;*/break;
          default:break;
      }
  }
  return ret;
...
unsigned char stream_file_trans(unsigned int id, unsigned char *buffer, unsigned long buf_len)
{
  ...
  for (int cnt = 0; cnt < pack_num; cnt++)
  {
    ...
    if(TUYA_SUCCESS == stream_trans(id, map_offset, buffer + map_offset, this_len))
    {
      //mcu正在升级中，不可以进行流服务传输
      //printf("is upgrade\n");
      return TUYA_SUCCESS;
