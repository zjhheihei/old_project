...
{    
    switch(wifiSendStatus)
    {
        ...
        case WIFI_SEND_STATUS_ALONE_DP:
        {
            ...
            #if defined(__RT1__) || defined(__RT3__) || defined(__RT81__) 
            ...
            #else   
            #if defined(__RT5__) 
            uint16_t mode = (uint16_t)ptSysPara->record.tempControlMode;
            if(2 == mode)
            {
                mode = 1;
            }
            ...
            #if defined(__HEATING__)
            #if defined(__RT1__) || defined(__RT3__) 
            uint16_t status = 0;
...
void app_wifi_importantDatChanged(void)
{
    ...
    if(ms_500 <= diffTick)
    {
        ...
        if(mode != ptSysPara->record.tempControlMode)
        {
            bool updatFlag = false;
            #ifndef __RT5__                             
            updatFlag = true;
            #else
            if(mode == TEMP_CONTROL_MODE_MANUAL)
            {
                if(TEMP_CONTROL_MODE_MANUAL != ptSysPara->record.tempControlMode)
                {
                    updatFlag = true;
                }
            ...
            #endif
            mode = ptSysPara->record.tempControlMode;   
            if(updatFlag)
            {
                updataFlagEnable = true;   
                sendDelayTime = GetSysTickMillisecond(); 
        ...
        switch(add)
        {
            case REG_CLOUD_ADDR_POWER_SWITCH:
            {
                ...
                else
                {
                    ...
                    #endif
                    #if defined(__RT1__) || defined(__RT2__) || defined(__RT3__) || defined(__RT4__) || defined(__RT5__) 
            ...
            {               
                ...
                if(TEMP_CONTROL_MODE_MANUAL == ptSys->record.tempControlMode)
                {
                    #if defined(__HEATING__)
                ...
                {
                    ptSys->record.tempControlMode = TEMP_CONTROL_MODE_TEMPORARY;
                }                 
                break;
            }
            ...
            case REG_CLOUD_ADDR_LIF_SET_TEMP:
            {
                ...
                app_con_tempLegalProtect(MAX_LIF_TEMP,MIN_LIF_TEMP,&ptSys->record.lifSetTemp);
                #endif
                break;               
            }
            case REG_CLOUD_ADDR_WORK_MODE:
            {
                ...
                #else   
                #if defined(__RT5__) 
                ptSys->record.tempControlMode = (tempControlMode_t)dat;       
                ptSys->blink_RTC_TEMP_flag = false;
                ...
                #ifndef __RT5__
                ptSys->record.tempControlMode = TEMP_CONTROL_MODE_MANUAL;
                #endif
                break;
            }
            ...
            {
                #ifdef __RT5__              
                static uint8_t debugBuf[64];
                uint8_t length = 0;
                uint8_t i = 0;
                length = raw_dp_updat_to_deceive(REG_CLOUD_ADDR_AI_PRO_OEM,debugBuf);   
                volatile static uint8_t debugLen[2];
                debugLen[0] = length;
                debugLen[1] = AI_PRG_LENGTH;
                if(length == AI_PRG_LENGTH)
                {
                    for(i = 0;i < (AI_PRG_LENGTH / 4) ;i++)                   
                    {
                        ptSys->record.interval[i].hour   = debugBuf[i * 4 + 0];
                        ptSys->record.interval[i].minute = debugBuf[i * 4 + 1];
                        int16_t temp = 0;
                        temp = debugBuf[i * 4 + 2];
                        temp <<= 8 ;
                        temp |= debugBuf[i * 4 + 3];
